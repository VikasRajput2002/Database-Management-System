...
ALTER table Class
ADD credits varchar(30);

Alter table Enrolled
ADD grade varchar(30);


create trigger update_credits1 after update of grade on enrolled
referencing new row as nrow
referencing old row as orow
for each row
when nrow.grade = 'F' or nrow.grade = null
begin atomic
 update student
 set total_credits = 0
 where student.snum = nrow.snum
end;



create trigger update_credits2 after update of grade on enrolled
referencing new row as nrow
referencing old row as orow
for each row
when nrow.grade != 'F' and nrow.grade != null 
begin atomic
 update student
 set total_credits = (select credits from class where nrow.cname = class.name)
 where student.snum = nrow.snum
end;
