...
from (
	...
	and faculty.fid = class.fid and class.name = enrolled.cname and enrolled.snum = student.snum)
) as stud(name1, age1)
...
where room = 'R128'or (name in (select name from Class, Enrolled 
where Class.name = Enrolled.cname and (Enrolled.snum in 
(select Enrolled.snum from Student, Enrolled 
where Student.snum = Enrolled.snum group by Student.snum having COUNT(Student.snum) < 3))));


/* 3rd */
select S.age, S.level
from Student S group by S.age, S.level having S.level in(select S1.level from Student S1
where S1.age = S.age group by S1.level, S1.age having  COUNT(*) >= ALL(select count(*)
from Student S2
where S1.age = S2.age
...
where fid in (select Faculty.fid from Faculty, Class 
where Faculty.fid =  Class.fid and name like "O%" and room = "R15" and name in 
(select Class.name from Class, Enrolled 
where Class.name = Enrolled.cname group by Class.name having count(snum) >= 1));
 
/* 5th */
select distinct sname from Student join Enrolled 
on Student.snum = Enrolled.snum join Class
on Enrolled.cname = Class.name 
where Class.room = "R15" and level = "SR";


/* 6th */
select distinct cname from  Enrolled join Student
where Student.snum != Enrolled.snum and (Enrolled.cname like "O%" or "A%" and "%s");


/* 7th */
select fname, Faculty.fid, count(Class.fid) 
as num_classes from Faculty left join Class 
on Faculty.fid = Class.fid group by Faculty.fid;
